            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in rows:
                    try:
                        ...
                        checkpoint_data = (
                            json.loads(row["checkpoint"]) if row["checkpoint"] else {}
                        )
                        ...
                        conversation = {
                            ...
                            "messages": checkpoint_data.get("channel_values", {}).get(
                                "messages", []
                            ),
    ...
    def store_insights_to_memory(
        ...
        try:
            ...
            for i, suggestion in enumerate(summary.get("improvement_suggestions", [])):
                ...
                self.store.put(
                    improvements_namespace, improvement_id, improvement_entry
                )
...
def main():
    ...
    parser.add_argument(
        "--date", type=str, help="Date to analyze (YYYY-MM-DD). Default: yesterday"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Print insights without storing them"
    )
